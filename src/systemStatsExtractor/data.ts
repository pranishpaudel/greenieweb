...
const calculateInJoules = (cpuPowerInMW: number, totalSec: number) => {
...
export const getSystemStats = (
  ...
  calculatedTotalEnergy: number;
  calculatedCo2Emission: number;
  combinedPowerInMW: number;
}> => {
  return new Promise((resolve, reject) => {
    ...
    exec(terminalCommand, (error, cmdOut, cmdErr) => {
      if (error) {
        ...
        const calculatedTotalEnergy = calculateInJoules(
          combinedPowerInMW,
          totalSec
        );
        ...
        const calculatedCo2Emission = calculateCarbonEmission(
