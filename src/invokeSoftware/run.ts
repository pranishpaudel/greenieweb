 ...
 */
export const turnOnApp = (appName: string) => {
  return new Promise<{
    energyConsumedWhileStartup: string;
    co2EmissionWhileStartup: string;
    powerInMWWhileStartup: string;
    energyConsumedAfterStartup: string;
    co2EmissionAfterStartup: string;
    powerInMWAfterStartup: string;
  }>((resolve, reject) => {
    ...
    const appInstance = exec(
      `open -a "${appName}"`,
      async (error, cmdOut, cmdErr) => {
        console.log(".................................");
        if (error) {
        ...
        try {
          const calculatedSystemStatsWithSoftwareWhileStartup =
            await getSystemStats(terminalCommand, singaporeCarbonIntensity);
    ...
    appInstance.on("exit", async (code) => {
      if (code === 0) {
        try {
          const calculatedSystemStatsWithSoftwareAfterStartup =
            await getSystemStats(terminalCommand, singaporeCarbonIntensity);
